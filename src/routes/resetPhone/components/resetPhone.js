...
import './resetPhone.scss'

class ResetPhoneView extends React.Component {
	static propTypes = {
	...
	sendCode() {
		...
		props.fetchList(
			'/firmware/valid_code/',
			{ querys: { account, v_type: 'reset_phone' } },
			'codeInfo',
			codeFilter
		)
	}
	...
	render() {
		...
		let { phoneError, vcodeError } = errorMessages
